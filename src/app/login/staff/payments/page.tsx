...
export default function StaffPaymentsPage() {
	...
	useEffect(() => {
		const refreshAccess = async () => {
			...
			setPaymentsAccess(getCurrentStaffModuleAccess('payments'));
			setModuleAccess(
	...
	const canEdit = paymentsAccess === 'edit';
	const handleNewPayment = () => {
		window.location.href = '/login/staff/payments/editor';
	};
		...
		<div className="min-h-screen bg-slate-50">
			...
			<main className="ml-56 min-h-screen">
				...
				<header className="sticky top-0 z-30 flex h-10 items-center justify-between border-b border-slate-200 bg-white px-5">
					<div>
						<p className="text-sm font-extrabold tracking-tight text-slate-900">Payments</p>
						{/* Access Level label removed to match admin UI */}
						<p className="text-[11px] text-slate-500">{canEdit ? 'You can add, edit, and remove payments.' : 'View-only access. Editing is disabled.'}</p>
					</div>
				...
				<div className="mx-auto w-full max-w-[1400px] p-4">
					<div className="rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
						<div className="flex flex-wrap items-center justify-between gap-3">
							<div>
								<p className="text-lg font-bold text-slate-900">Payments module</p>
								<p className="text-sm text-slate-600">
									{canEdit ? 'Edit mode is active. You can manage payment records.' : 'View mode is active. You can review payment records only.'}
								</p>
							</div>
							<button
								type="button"
						...
						<div className="mt-4 grid gap-3 md:grid-cols-3">
							<div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">View: list and inspect payments.</div>
							<div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Edit: add or change payment records.</div>
							<div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Remove: available only in edit mode.</div>
						</div>
						<div className="mt-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-slate-500">
							Payments data is not loaded yet.
						</div>
					</div>
				</div>
			</main>
