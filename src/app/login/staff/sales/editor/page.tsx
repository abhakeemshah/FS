...
import { getCurrentStaffModuleAccess, type StaffAccessLevel, SALES_BILLS_STORAGE_KEY, writeStoredArray, type BillRecord } from '../../../../../lib/sales-utils';
import { syncLocalStaffMetaWithServer } from '../../../../../lib/staff-auth';

export default function StaffSalesEditorPage() {
	...
	useEffect(() => {
		const checkAccess = async () => {
			...
			const access = getCurrentStaffModuleAccess('sales');
			if (access !== 'edit') {
				router.push('/login/staff/sales');
				return;
			}
			setSalesAccess(access);
			setAccessReady(true);
		};
	...
	const handleSaveInvoice = () => {
		...
		const bills = [newBill];
		writeStoredArray<BillRecord>(SALES_BILLS_STORAGE_KEY, bills);
			...
			<div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
				<div className="mb-6 flex items-center justify-between">
					<div>
						...
						<p className="text-sm text-slate-600">Add invoice details and line items.</p>
					</div>
					<Link href="/login/staff/sales" className="rounded-lg border border-slate-300 bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200">
						Back
					</Link>
				</div>
				...
				<div className="space-y-4">
					<div className="space-y-3 rounded-lg border border-slate-300 bg-slate-50 p-4">
						<h3 className="text-sm font-bold uppercase tracking-wide text-slate-700">Invoice Details</h3>
						<input
							...
							onChange={(e) => setFormData((c) => ({ ...c, invoiceNumber: e.target.value }))}
						/>
						<input
							className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-800 outline-none focus:border-blue-400"
