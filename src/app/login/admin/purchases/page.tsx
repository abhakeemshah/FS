...
const createBlankLineItem = (): DraftLineItem => ({
  product: '', boxes: '', piecesPerBox: '', loosePieces: '', unitCost: '',
});
...
function PurchaseDropdown({ value, options, onChange, placeholder, buttonClassName = '', menuClassName = '' }: PurchaseDropdownProps) {
  ...
  const scheduleClose = () => {
    cancelClose();
    closeTimerRef.current = setTimeout(() => { setOpen(false); closeTimerRef.current = null; }, 120);
  };

  return (
    <div className="relative" onMouseEnter={() => { cancelClose(); setOpen(true); }} onMouseLeave={scheduleClose}>
      <button type="button" onClick={() => { cancelClose(); setOpen((c) => !c); }} className={buttonClassName} aria-haspopup="listbox" aria-expanded={open}>
        <span className="truncate">{selectedValue}</span>
        <span className="pointer-events-none text-slate-500">{open ? '▴' : '▾'}</span>
      </button>
      {open ? (
        <div className={`absolute left-0 right-0 top-[calc(100%+4px)] z-30 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_14px_30px_rgba(15,23,42,0.14)] ${menuClassName}`} role="listbox" onMouseEnter={cancelClose} onMouseLeave={scheduleClose}>
          {options.map((option) => {
            const isSelected = option === value;
            return (
              <button key={option} type="button" role="option" aria-selected={isSelected}
                onClick={() => { cancelClose(); onChange(option); setOpen(false); }}
...
export default function AdminPurchasesPage() {
  ...
  useEffect(() => {
    ...
    if (!staff && !hasAdminSession()) { router.push('/access'); }
  }, [router]);
  ...
  const loadPurchases = async () => {
    try {
      const res = await fetch('/api/purchases', { cache: 'no-store', credentials: 'include' });
      if (res.ok) {
  ...
  const updateDraftLineItem = (key: keyof DraftLineItem, value: string) => {
    setForm((current) => ({ ...current, draftLineItem: { ...current.draftLineItem, [key]: value } }));
  };
  ...
  const addLineItem = () => {
    ...
    if (!product) { setError('Product name is required.'); return; }
    if (!Number.isFinite(boxes) || boxes < 0 || !Number.isFinite(piecesPerBox) || piecesPerBox < 0 || !Number.isFinite(loosePieces) || loosePieces < 0) {
      setError('Boxes, pieces per box, and loose pieces must be valid numbers.'); return;
    }
    if (!Number.isFinite(unitCost) || unitCost < 0) { setError('Unit cost must be a valid number.'); return; }

    setForm((current) => ({
      ...current,
      lineItems: [...current.lineItems, { product, boxes: String(boxes), piecesPerBox: String(piecesPerBox), loosePieces: String(loosePieces), unitCost: String(unitCost) }],
      draftLineItem: createBlankLineItem(),
    }));
  };
  ...
  const closeViewPurchase = () => setViewPurchase(null);

  const askDeletePurchase = (purchase: PurchaseRecord) => setDeleteTarget(purchase);
  const cancelDeletePurchase = () => setDeleteTarget(null);

  const confirmDeletePurchase = async () => {
    if (!deleteTarget) return;
    try {
      const res = await fetch(`/api/purchases?id=${deleteTarget.id}`, { method: 'DELETE', credentials: 'include' });
      const data = await res.json();
      if (data.success) {
        setPurchases((current) => current.filter((p) => p.id !== deleteTarget.id));
      }
    } catch { /* ignore */ }
  ...
  const exportPurchase = (purchase: PurchaseRecord) => {
    const html = buildPrintablePurchaseInvoice({
      ...
      recordedBy: purchase.recordedBy ?? undefined,
    });
    setPreviewHtml(html);
  ...
  const savePurchase = async () => {
    ...
    const items = form.lineItems.map((item) => {
      ...
      return { productName: item.product.trim(), boxes, piecesPerBox, loosePieces, unitCost, totalUnits, lineTotal };
    });
    ...
    try {
      ...
      setError('Network error saving purchase.');
    }
  };
    ...
    <AdminShell active="purchases" title="Purchases">
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between gap-3 border-b border-blue-100 bg-blue-50 px-4 py-3">
          ...
          {canEdit ? (
            <button type="button" onClick={openNewPurchase} className="rounded-lg border border-blue-600 bg-blue-600 px-4 py-2 text-xs font-bold text-white transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.01] hover:bg-blue-700 active:translate-y-0 active:scale-95">New Purchase</button>
          ) : null}
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {!mounted ? (
                <tr><td className="px-4 py-4 text-slate-500" colSpan={5}>No purchases yet.</td></tr>
      ...
      {mounted && isModalOpen ? (
        <AppModal open={isModalOpen} onClose={closeModal} overlayClassName="items-start" cardClassName="purchase-new-purchase-card flex max-h-[calc(100dvh-2rem)] max-w-5xl flex-col overflow-hidden rounded-2xl border border-slate-300 shadow-2xl transition-all duration-200 ease-out sm:max-h-[92vh] translate-y-0 scale-100 opacity-100">
          <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={closeModal} className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-[11px] font-semibold text-rose-700 transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.02] hover:bg-rose-100 active:translate-y-0 active:scale-95">Close</button>
            </div>
          </div>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {activePurchase.items.length ? activePurchase.items.map((item, index) => (
                        <tr key={`${activePurchase.id}-${index}`}>
                        ...
                        <tr><td className="px-3 py-3 text-slate-500" colSpan={3}>No line items saved for this record.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 rounded-2xl border border-slate-300 bg-slate-50 p-4 shadow-sm">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    <input value={form.supplierName} onChange={(e) => setForm((c) => ({ ...c, supplierName: e.target.value }))} placeholder="Supplier Name" className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                    <input value={form.sourceName} onChange={(e) => setForm((c) => ({ ...c, sourceName: e.target.value }))} placeholder="Source Name" className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                    <input value={form.purchaseReference} onChange={(e) => setForm((c) => ({ ...c, purchaseReference: e.target.value }))} placeholder="Reference Number" className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                    <input type="date" value={form.purchaseDate} onChange={(e) => setForm((c) => ({ ...c, purchaseDate: e.target.value }))} className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                    <input type="time" value={form.purchaseTime} onChange={(e) => setForm((c) => ({ ...c, purchaseTime: e.target.value }))} className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                    <PurchaseDropdown value={form.paymentMethod} options={paymentMethodOptions} onChange={(paymentMethod) => setForm((c) => ({ ...c, paymentMethod }))} placeholder="Payment Method" buttonClassName="flex h-10 w-full items-center justify-between rounded-xl border border-blue-200 bg-gradient-to-b from-white to-blue-50 px-3 text-left text-[13px] font-semibold text-slate-800 shadow-sm transition-all duration-200 hover:border-blue-300 hover:shadow focus:border-blue-400 focus:outline-none focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                    <PurchaseDropdown value={form.status} options={purchaseStatusOptions} onChange={(status) => setForm((c) => ({ ...c, status }))} placeholder="Status" buttonClassName="flex h-10 w-full items-center justify-between rounded-xl border border-slate-300 bg-white px-3 text-left text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 hover:border-blue-300 hover:shadow focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" menuClassName="max-h-56 overflow-auto" />
                    <input value={form.transportCost} onChange={(e) => setForm((c) => ({ ...c, transportCost: e.target.value }))} placeholder="Transport Cost" className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-medium text-slate-800 outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
                  </div>
                </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                      ...
                      <p className="mt-1 text-base font-extrabold text-slate-900">{formatMoney(form.lineItems.reduce((sum, item) => {
                        const b = Number(item.boxes) || 0; const ppb = Number(item.piecesPerBox) || 0; const lp = Number(item.loosePieces) || 0; const uc = Number(item.unitCost) || 0;
                        return sum + (b * ppb + lp) * uc;
                      }, 0))}</p>
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                ...
                <textarea value={form.notes} onChange={(e) => setForm((c) => ({ ...c, notes: e.target.value }))} placeholder="Notes" rows={3} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition-all duration-200 focus:border-blue-400 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" />
              </div>
            )}
          </div>
      ...
      {isPreviewOpen && previewHtml ? (
        <AppModal open={isPreviewOpen} onClose={() => { setIsPreviewOpen(false); setPreviewHtml(null); }} cardClassName="w-full max-w-4xl max-h-[90vh] overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => { const w = window.open('', '_blank'); if (!w) { setError('Popup blocked.'); return; } w.document.open(); w.document.write(previewHtml); w.document.close(); w.focus(); }} className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold">Open in New Tab</button>
              <button type="button" onClick={() => { const iframe = document.getElementById('print-preview-iframe') as HTMLIFrameElement | null; if (iframe && iframe.contentWindow) { try { iframe.contentWindow.focus(); iframe.contentWindow.print(); } catch (err) { setError('Print failed: ' + String(err)); } } }} className="rounded-lg border border-blue-600 bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white">Download PDF</button>
      ...
      {viewPurchase ? (
        <AppModal open={Boolean(viewPurchase)} onClose={closeViewPurchase} cardClassName="w-full max-w-3xl max-h-[86vh] overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
            <div><h3 className="text-sm font-bold text-slate-900">Purchase View</h3><p className="text-[11px] text-slate-500">Read-only purchase details</p></div>
          ...
          <div className="max-h-[calc(86vh-72px)] space-y-4 overflow-y-auto p-4">
            ...
            <div className="ml-auto w-full max-w-xs rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
              <div className="flex items-center justify-between"><span className="text-slate-600">Subtotal</span><span className="font-semibold text-slate-900">{formatMoney(viewPurchase.subtotal)}</span></div>
              ...
              <div className="mt-2 flex items-center justify-between border-t border-slate-200 pt-2"><span className="font-bold text-slate-700">Total</span><span className="text-lg font-black text-slate-900">{formatMoney(viewPurchase.total)}</span></div>
            </div>
          </div>
        </AppModal>
      ) : null}
      ...
      {deleteTarget ? (
        <AppModal open={Boolean(deleteTarget)} onClose={cancelDeletePurchase} cardClassName="w-full max-w-sm">
          <div className="p-4">
            ...
            <p className="mt-1 text-xs text-slate-600">Delete <span className="font-semibold text-slate-800">{deleteTarget.purchaseNumber}</span> from <span className="font-semibold text-slate-800">{deleteTarget.supplierName}</span>?</p>
