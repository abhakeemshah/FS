...
type ProductRecord = {
  ...
  categoryId: string;
  bio: string | null;
  imageUrls: string | null; // JSON string array
  price: number;
  costPrice: number;
  stock: number;
  status: string;
...
type ListRecord = {
  ...
  createdAt: string;
  items: { id: string; listId: string; productId: string }[];
  _count?: { items: number };
};
...
const defaultLandingHeroSettings = {
...
const defaultLandingSectionVisibility = { hot: true, more: true };

type LandingSectionVisibility = { hot: boolean; more: boolean };
...
const parseImageUrls = (imageUrls: string | string[] | null | undefined): string[] => {
  if (!imageUrls) return [];
...
const normalizeLandingHeroForm = (settings: Partial<HeroRecord> | null | undefined): LandingHeroFormState => ({
...
const buildImageUrls = (form: ProductFormState) =>
  [form.imageUrl, form.imageUrl2, form.imageUrl3, form.imageUrl4]
...
export default function AdminProductsPage({
  ...
  const [heroForm, setHeroForm] = useState<LandingHeroFormState>(normalizeLandingHeroForm(initialHero));
  ...
  const [landingSectionVisibility, setLandingSectionVisibility] = useState<LandingSectionVisibility>(
    defaultLandingSectionVisibility,
  );
  ...
  const initialHeroFormRef = useRef(heroForm);
  ...
  const refreshData = async () => {
    try {
      const [productsRes, categoriesRes, listsRes, heroRes] = await Promise.all([
        fetch('/api/products', { cache: 'no-store', credentials: 'include' }),
        fetch('/api/categories', { cache: 'no-store', credentials: 'include' }),
        fetch('/api/product-lists', { cache: 'no-store', credentials: 'include' }),
        fetch('/api/landing-hero', { cache: 'no-store', credentials: 'include' }),
      ]);

      const [productsData, categoriesData, listsData, heroData] = await Promise.all([
        productsRes.json(),
        categoriesRes.json(),
        listsRes.json(),
        heroRes.json(),
      ...
      if (listsData.success) setLists(listsData.lists);
      if (heroData.success && heroData.hero) {
        const normalized = normalizeLandingHeroForm(heroData.hero);
        setHeroForm(normalized);
        initialHeroFormRef.current = normalized;
      }
    } catch {
  ...
  const totalActive = useMemo(() => products.filter((p) => p.status === 'active').length, [products]);
  const totalLanding = useMemo(() => products.filter((p) => p.showOnLanding).length, [products]);
  const totalExtraLanding = useMemo(() => products.filter((p) => p.showOnExtraLanding).length, [products]);
  ...
  const saveCategory = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    openConfirm(
      ...
      editingCategoryId ? 'Do you want to update this category?' : 'Do you want to create this category?',
      async () => {
        const body = {
          slug: editingCategoryId ? undefined : slug,
          name: trimmedName,
        ...
        try {
          if (editingCategoryId) {
            const res = await fetch(`/api/categories?id=${editingCategoryId}`, {
              method: 'PATCH',
            ...
            if (data.success) {
              setCategories((prev) => prev.map((c) => (c.id === editingCategoryId ? data.category : c)));
            }
          } else {
          ...
          setEditingCategoryId(null);
          setNotice(`${editingCategoryId ? 'Category updated' : 'Category created'}: ${trimmedName}`);
        } catch {
          setNotice('Error saving category. Please try again.');
        }
      },
    );
  };
  ...
  const deleteCategory = (categoryId: string, categoryNameValue: string) => {
    openConfirm('Delete category?', `Delete category "${categoryNameValue}" and all linked products?`, async () => {
      try {
  ...
  const saveProduct = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const skuTaken = products.some(
      (p) => p.sku.toLowerCase() === sku.toLowerCase() && p.id !== editingProductId,
    );
    if (skuTaken) {
    ...
    openConfirm(
      ...
      editingProductId ? 'Update this product?' : 'Create this product?',
      async () => {
        try {
          if (editingProductId) {
            const res = await fetch(`/api/products?id=${editingProductId}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
              body: JSON.stringify(body),
          ...
          setNotice(`Product ${editingProductId ? 'updated' : 'added'}: ${name}`);
          setIsProductModalOpen(false);
          setEditingProductId(null);
          setProductForm(emptyProductForm());
        } catch {
          setNotice('Error saving product. Please try again.');
        }
      },
    );
  };
  ...
  const deleteProduct = (productId: string, productName: string) => {
    openConfirm('Delete product?', `Delete product "${productName}" everywhere?`, async () => {
      try {
        ...
        if (data.success) {
          ...
          setNotice(`Deleted product: ${productName}`);
        }
      } catch {
        setNotice('Error deleting product.');
      }
    });
  };

  const toggleLanding = (productId: string) => {
    openConfirm('Update homepage visibility?', 'Toggle whether this product shows on the homepage?', async () => {
      ...
      try {
        const res = await fetch(`/api/products?id=${productId}`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ showOnLanding: !product.showOnLanding }),
        });
        const data = await res.json();
        if (data.success) {
        ...
        setNotice('Error updating product.');
      }
    });
  };

  const toggleExtraLanding = (productId: string) => {
    openConfirm('Update secondary landing visibility?', 'Toggle whether this product shows in the second homepage list?', async () => {
      const product = products.find((p) => p.id === productId);
      if (!product) return;
      try {
        const res = await fetch(`/api/products?id=${productId}`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ showOnExtraLanding: !product.showOnExtraLanding }),
        });
        const data = await res.json();
        if (data.success) {
          setProducts((prev) => prev.map((p) => (p.id === productId ? data.product : p)));
  ...
  const [lists, setLists] = useState<ListRecord[]>(initialLists);
  ...
  const landingVisibilityStats = useMemo(() => {
    const hiddenLandingProducts = products.filter((p) => !p.showOnLanding).length;
    const hiddenExtraLandingProducts = products.filter((p) => !p.showOnExtraLanding).length;
    const visibleLandingLists = lists.filter((l) => l.visibleOnLanding !== false && l.items.length > 0).length;
    const hiddenLandingLists = lists.filter((l) => l.visibleOnLanding === false).length;
    const emptyLandingLists = lists.filter((l) => l.items.length === 0).length;
  ...
  const refreshLists = async () => {
    try {
      const res = await fetch('/api/product-lists', { cache: 'no-store', credentials: 'include' });
      const data = await res.json();
  ...
  const setSelectedList = (listId: string | null) => {
    ...
    if (listId) {
      setLandingSectionVisibility((prev) => ({ ...prev, more: true }));
    }
  };
  ...
  const openManageList = (list: ListRecord) => {
    setEditingListId(list.id);
    setListName(list.name);
    setListProductIds(list.items.map((i) => i.productId));
    setIsListModalOpen(true);
  };
  ...
  const saveList = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    openConfirm(
      ...
      editingListId ? 'Update this product list?' : 'Create this product list?',
      async () => {
        try {
          const body = { name: trimmed, productIds: listProductIds };
          if (editingListId) {
            const res = await fetch(`/api/product-lists?id=${editingListId}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
              body: JSON.stringify(body),
            ...
            const data = await res.json();
            if (data.success) {
              setLists((prev) => prev.map((l) => (l.id === editingListId ? data.list : l)));
              setSelectedListIdState(editingListId);
            }
          } else {
            ...
            if (data.success) {
              ...
              setSelectedListIdState(data.list.id);
              setLandingSectionVisibility((prev) => ({ ...prev, more: true }));
            }
          }

          // Activate all products in the list
          setProducts((prev) =>
            prev.map((p) =>
  ...
  const deleteList = (listId: string) => {
    ...
    openConfirm('Delete list everywhere?', `Delete list "${target.name}" everywhere?`, async () => {
      try {
        const res = await fetch(`/api/product-lists?id=${listId}`, {
          method: 'DELETE',
          credentials: 'include',
        });
        const data = await res.json();
        if (data.success) {
          ...
          if (selectedListId === listId) setSelectedListIdState(null);
          setNotice(`Deleted list: ${target.name}`);
        }
      } catch {
        setNotice('Error deleting list.');
      }
    });
  ...
  const saveLandingHero = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    openConfirm('Save landing hero?', 'Save the landing hero settings?', async () => {
      ...
      const nextHero = {
        ...
        backgroundColor: defaultLandingHeroSettings.backgroundColor,
        overlayOpacity: Number.isFinite(opacityNum)
      ...
      try {
        ...
        const data = await res.json();
        if (data.success) {
          setHeroForm(normalizeLandingHeroForm(data.hero));
          setNotice('Landing hero updated.');
          setIsEditingHeroSettings(false);
        }
      } catch {
        setNotice('Error saving landing hero.');
      }
    });
  };

  const cancelHeroEdit = () => {
    setHeroForm(initialHeroFormRef.current);
    setIsEditingHeroSettings(false);
  };
  ...
  const openLandingListEditor = (target: 'hot' | 'more') => {
    ...
    if (target === 'hot') {
      setLandingListEditorProductIds(products.filter((p) => p.showOnLanding).map((p) => p.id));
      setLandingListEditorInactiveIds(
        products.filter((p) => p.showOnLanding && p.status === 'draft').map((p) => p.id),
      );
    } else {
      setLandingListEditorProductIds(selectedList?.items.map((i) => i.productId) ?? []);
      setLandingListEditorInactiveIds(
        (selectedList?.items ?? [])
          .filter((item) => products.find((p) => p.id === item.productId)?.status === 'draft')
  ...
  const saveLandingListEditor = async () => {
    ...
    // Update each product status and landing visibility via API
    const productUpdates: { id: string; showOnLanding: boolean; status: ProductStatus }[] = [];
    ...
    for (const product of products) {
      ...
      if (landingListEditorTarget === 'hot') {
        if (!isSelected) {
          if (product.showOnLanding) {
            productUpdates.push({ id: product.id, showOnLanding: false, status: normalizeStatus(product.status) });
          }
          continue;
        }
        productUpdates.push({
    ...
    try {
      ...
      await Promise.all(
        productUpdates.map((update) =>
          fetch(`/api/products?id=${update.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({
              showOnLanding: update.showOnLanding,
      ...
      // If editing a 'more' list, update the list's product IDs
      if (landingListEditorTarget === 'more' && selectedList) {
        const res = await fetch(`/api/product-lists?id=${selectedList.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ productIds: landingListEditorProductIds }),
        });
        const listData = await res.json();
        if (listData.success) {
          setLists((prev) => prev.map((l) => (l.id === selectedList.id ? listData.list : l)));
        }
      }

      // Refresh products from server to get consistent state
      const productsRes = await fetch('/api/products', { cache: 'no-store', credentials: 'include' });
      const productsData = await productsRes.json();
      if (productsData.success) setProducts(productsData.products);

      setNotice(
        `${landingListEditorTarget === 'hot' ? "What's Hot Right Now" : 'More From Admin'} updated.`,
      );
    } catch {
  ...
  const disableHotRightNowList = () => {
    openConfirm("Disable What's Hot Right Now?", "Remove its items from the homepage? This will mark the items as draft.", async () => {
      try {
        await Promise.all(
          products
            ...
            .map((p) =>
              fetch(`/api/products?id=${p.id}`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({ showOnLanding: false, status: 'draft' }),
              }),
            ),
        );
        ...
        setNotice('Error disabling hot list.');
      }
    });
  };

  const deleteHotRightNowList = () => {
    openConfirm("Delete What's Hot Right Now?", "This will remove the section and its products from all views.", async () => {
      try {
        const landingProductIds = products.filter((p) => p.showOnLanding).map((p) => p.id);
        const errors: string[] = [];

        if (landingProductIds.length) {
          const results = await Promise.allSettled(
            landingProductIds.map((id) =>
          ...
          results.forEach((r) => { if (r.status === 'rejected') errors.push('product delete'); });
        }

        // Also delete all lists
        const listResults = await Promise.allSettled(
          lists.map((l) =>
            fetch(`/api/product-lists?id=${l.id}`, {
              method: 'DELETE',
              credentials: 'include',
            }),
          ),
        );
        listResults.forEach((r) => { if (r.status === 'rejected') errors.push('list delete'); });

        setLandingSectionVisibility((prev) => ({ ...prev, hot: false }));
        ...
        setNotice(errors.length ? "What's Hot Right Now deleted (with some errors)." : "What's Hot Right Now deleted everywhere.");
      } catch {
        setNotice('Error deleting hot list.');
      }
    });
  };

  const editMoreFromAdminList = (list: ListRecord) => {
    openManageList(list);
  };

  const disableMoreFromAdminList = (list: ListRecord) => {
    openConfirm('Disable list on landing?', `Disable "${list.name}" on landing?`, async () => {
      try {
        const res = await fetch(`/api/product-lists?id=${list.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ visibleOnLanding: false }),
        });
        const data = await res.json();
        if (data.success) {
        ...
        setNotice('Error disabling list.');
      }
    });
  };

  const enableMoreFromAdminList = (list: ListRecord) => {
    openConfirm('Enable list on landing?', `Enable "${list.name}" on landing?`, async () => {
      try {
        const res = await fetch(`/api/product-lists?id=${list.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ visibleOnLanding: true }),
        });
        const data = await res.json();
        if (data.success) {
          setLists((prev) => prev.map((l) => (l.id === list.id ? data.list : l)));
        ...
        setNotice('Error enabling list.');
      }
    });
  };

  const deleteMoreFromAdminList = (list: ListRecord) => {
    openConfirm('Delete list everywhere?', `Delete list "${list.name}" everywhere?`, async () => {
      try {
        const res = await fetch(`/api/product-lists?id=${list.id}`, {
          method: 'DELETE',
          credentials: 'include',
        ...
        if (data.success) {
          ...
          setLandingSectionVisibility((prev) => ({
            ...prev,
            more: lists.filter((l) => l.id !== list.id).length > 0,
          }));
  ...
  // ─── JSX (UI preserved from original) ─────────────────────────────────────
  ...
  return (
    <AdminShell active="products" title="Products">
                      ...
                      <div className="flex flex-wrap items-start gap-3">
                        {products.filter((p) => p.showOnLanding).length ? (
                          products
                            .filter((p) => p.showOnLanding)
                            .map((p) => {
                              const urls = parseImageUrls(p.imageUrls);
                              return (
                                <div key={p.id} className="w-[140px] rounded-md border border-slate-200 bg-white p-2 text-xs">
                                  <div className="h-20 w-full overflow-hidden rounded-md bg-slate-50">
                                    {urls[0] ? <img src={urls[0]} alt={p.name} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-slate-400">No img</div>}
                                  </div>
                                  <div className="mt-2 truncate font-semibold text-slate-900">{p.name}</div>
                      ...
                      return (
                        <div key={list.id}>
                          <div className="grid grid-cols-1 gap-2 border-t border-slate-200 px-3 py-2 text-xs sm:grid-cols-[1.4fr_0.7fr_1.9fr] sm:items-center sm:gap-0">
                            ...
                            <p className="text-slate-700">{list.items.length}</p>
                            <div className="flex flex-wrap items-center justify-start gap-2 sm:justify-end">
                              {canEdit ? (
                                <>
          ...
          <div className="space-y-2 p-3">
            {categories.length ? (
              categories.map((category) => {
                const linkedProducts = products.filter((p) => p.categoryId === category.id).length;
                        ...
                        return (
                          <tr key={product.id} className="transition-colors hover:bg-slate-50">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-3">
                                {urls[0] ? (
                                  <img src={urls[0]} alt={product.name} className="h-10 w-10 rounded-md border border-slate-200 object-cover" />
                                ) : (
      ...
      <AppModal open={isListModalOpen} onClose={() => setIsListModalOpen(false)} cardClassName="w-full max-w-4xl overflow-hidden">
        ...
        <form className="max-h-[70vh] space-y-3 overflow-y-auto p-4" onSubmit={saveList}>
          <label className="block text-xs font-semibold text-slate-600">
            List Name
